...
map<string, bool> GStreamer::publishRunData(GOptions *gopts, vector<GEventData*> *runData) {
	...
	for(auto eventData: (*runData)) {
		...
		for (auto& [detectorName, gDataCollection] : *eventData->getDataCollection() ) {
			...
			gstreamReport[dnameKey] = publishEventTrueInfoData(gDataCollection->getTrueInfoData());

			// publish digitized data
			dnameKey = detectorName + "__Digitized";
			gstreamReport[dnameKey] = publishEventDigitizedData(gDataCollection->getDigitizedData());
		}
